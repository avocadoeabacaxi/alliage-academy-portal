...
import { LayoutDashboard, FileText, PlusCircle, Users, LogOut, Menu, X, Search, PanelLeftClose, PanelLeftOpen, Activity, History, ClipboardList, Edit, User as UserIcon, ChevronDown, Settings, BarChart3, ContactRound, UserRoundPlus, MessageSquare } from 'lucide-react';
...
export default function Layout() {
  ...
  const navGroups = [
    ...
    },
    {
      ...
      items: [
        ...
        { path: '/quick-request', label: lang === 'en' ? 'Chat request' : lang === 'es' ? 'Solicitud por chat' : 'Solicitação por chat', icon: MessageSquare, roles: ['admin', 'educador', 'gerente_regional', 'solicitante'] },
  ...
  const isActive = (path) => {
    ...
    if (path === '/quick-request') return location.pathname === '/quick-request';
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 flex flex-col bg-gradient-to-b from-[#003B5C] via-[#003553] to-[#002840] text-white transition-all duration-300 lg:rounded-br-3xl ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} ${collapsed ? 'w-20' : 'w-56'}`}>
        ...
        <div className="flex items-center justify-center px-4 py-4 border-b border-white/10">
          <img src="https://media.base44.com/images/public/6a3bbaf52679642f9d0e9feb/3f49984b2_logotraininnniinin.png" alt="Alliage" className={`flex-shrink-0 ${collapsed ? 'h-10' : 'h-12'} w-auto`} />
