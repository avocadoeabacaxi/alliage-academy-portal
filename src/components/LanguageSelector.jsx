  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-2 rounded-lg hover:bg-white/10 transition-colors text-sm font-semibold text-white border border-white/20 hover:border-white/40"
      >
        <span className="text-base">{LANG_FLAGS[lang]}</span>
        {!compact && <span>{LANG_LABELS[lang]}</span>}
        <svg className={`w-4 h-4 text-white/70 transition-transform ${open ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
