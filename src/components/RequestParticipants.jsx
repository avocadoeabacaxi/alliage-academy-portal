        ...
        <div className="space-y-2 mb-3">
          {participants.map((p, i) => (
            <div key={i} className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 bg-slate-50 rounded-xl p-3">
              ...
              <span className="text-xs font-medium text-[#003B5C]">{p.attendance_mode || 'Presencial'}</span>
              </div>
          ))}
        </div>
      )}
      ...
      {adding ? (
        <div className="border-t border-slate-100 pt-3 space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setAdding(false); setForm({ name: '', phone: '', email: '', attendance_mode: 'Presencial' }); }} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors">
              <X className="w-4 h-4" />
              {t('common.cancel')}
            </button>
          </div>
