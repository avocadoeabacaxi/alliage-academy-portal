  ...
  if (!req) {
    return (
      <div className="p-6 text-center">
        ...
        <button onClick={() => navigate('/requests')} className="mt-3 text-blue-600 hover:underline">{t('nav.requests')}</button>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 lg:p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-5">
        <button onClick={() => navigate('/requests')} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            <h1 className="text-xl font-bold text-slate-900">{req.request_id}</h1>
      ...
      <Section title={t('detail.trainingDetails')} icon={FileText}>
        <div className="space-y-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-800 bg-slate-50 rounded-lg p-3">{tf(req.training_focus)}</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-1">{t('form.audience')}</p>
              <div className="flex flex-wrap gap-1">
                {(req.audience || []).map(a => <span key={a} className="px-2 py-0.5 text-xs bg-blue-50 text-blue-700 rounded">{a}</span>)}
              </div>
            </div>
      ...
      <Section title={t('detail.justification')} icon={Star}>
        <div className="space-y-3">
          <div>
            ...
            <p className="text-sm text-slate-800 bg-slate-50 rounded-lg p-3">{tf(req.justification)}</p>
          </div>
          {(req.specific_problems || []).length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {req.specific_problems.map(p => <span key={p} className="px-2 py-0.5 text-xs bg-amber-50 text-amber-700 rounded">{p}</span>)}
              </div>
            </div>
          )}
          {(req.expected_impacts || []).length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {req.expected_impacts.map(p => <span key={p} className="px-2 py-0.5 text-xs bg-emerald-50 text-emerald-700 rounded">{p}</span>)}
              </div>
            </div>
          )}
          {tf(req.consequence_60_days) && (
            <div>
              ...
              <p className="text-sm text-slate-800 bg-slate-50 rounded-lg p-3">{tf(req.consequence_60_days)}</p>
            </div>
          )}
        </div>
      </Section>
      ...
      <Section title={t('detail.urgency')} icon={Calendar}>
        ...
        {tf(req.deadline_reason) && (
          <div className="mt-3">
            ...
            <p className="text-sm text-slate-800 bg-slate-50 rounded-lg p-3">{tf(req.deadline_reason)}</p>
          </div>
        )}
        ...
        <div className="space-y-3">
          {tf(req.educator_analysis) && (
            <div>
              ...
              <p className="text-sm text-slate-800 bg-slate-50 rounded-lg p-3">{tf(req.educator_analysis)}</p>
            </div>
          )}
          {req.date_stage1 && (
          ...
          {canReviewStage1 && req.decision_stage1 === 'Pendente' && (
            <div className="border-t border-slate-100 pt-3 space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={() => handleDecision(1, 'Aprovado')} disabled={saving || !educatorAnalysis} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 disabled:opacity-40 transition-colors">
                  {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                  {t('common.approve')}
                </button>
                <button onClick={() => handleDecision(1, 'Rejeitado')} disabled={saving} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-red-500 rounded-lg hover:bg-red-600 disabled:opacity-40 transition-colors">
                  <X className="w-4 h-4" />
                  {t('common.reject')}
                </button>
              </div>
          ...
          <div className="space-y-3">
            {tf(req.manager_analysis) && (
              <div>
                ...
                <p className="text-sm text-slate-800 bg-slate-50 rounded-lg p-3">{tf(req.manager_analysis)}</p>
              </div>
            )}
            {req.date_stage2 && (
              <div className="flex items-center gap-4 text-sm">
            ...
            {canReviewStage2 && req.decision_stage2 === 'Pendente' && (
              <div className="border-t border-slate-100 pt-3 space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleDecision(2, 'Aprovado')} disabled={saving || !managerAnalysis} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 disabled:opacity-40 transition-colors">
                    {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                    {t('common.approve')}
                  </button>
                  <button onClick={() => handleDecision(2, 'Rejeitado')} disabled={saving || !rejectionReason} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-red-500 rounded-lg hover:bg-red-600 disabled:opacity-40 transition-colors">
                    <X className="w-4 h-4" />
                    {t('common.reject')}
                  </button>
                </div>
          ...
          <div className="space-y-3">
            ...
            {tf(req.execution_notes) && (
              <div>
                ...
                <p className="text-sm text-slate-800 bg-slate-50 rounded-lg p-3">{tf(req.execution_notes)}</p>
              </div>
            )}
            {tf(req.final_notes) && (
              <div>
                ...
                <p className="text-sm text-slate-800 bg-slate-50 rounded-lg p-3">{tf(req.final_notes)}</p>
              </div>
            )}

            {/* Educator execution form */}
            {canEditExecution && !req.training_completed_date && (
              <div className="border-t border-slate-100 pt-3 space-y-3">
                ...
                <button onClick={handleCompleteTraining} disabled={saving} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 disabled:opacity-40 transition-colors">
                  {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
            ...
            {canCloseCycle && req.training_completed_date && !tf(req.final_notes) && req.status !== 'Concluído' && (
              <div className="border-t border-slate-100 pt-3 space-y-3">
                ...
                <button onClick={handleCloseCycle} disabled={saving} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-40 transition-colors">
                  {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                  {t('detail.closeCycle')}
                </button>
              </div>
      ...
      {req.decision_stage2 === 'Aprovado' && canGenerateSurvey && (
        <Section title={t('detail.survey')} icon={Star}>
          <div className="space-y-3">
            {!survey ? (
              <button onClick={handleGenerateSurvey} disabled={generatingSurvey} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-purple-600 rounded-lg hover:bg-purple-700 disabled:opacity-40 transition-colors">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 bg-slate-50 rounded-lg p-3">
                  ...
                  <button onClick={copySurveyLink} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors">
