...
export default async function(req) {
  try {
    ...
    if (body?.event && (record?.status !== 'Aprovado Etapa 2' || body?.old_data?.status === 'Aprovado Etapa 2')) return Response.json({ skipped: true, reason: 'status not newly approved' });
    ...
    const message = buildEmail('approval', language, { name: record.requester_name, requestId: record.request_id, product: record.product_name, educator: record.educator_name, date: formatDate(record.training_scheduled_date, language) });
