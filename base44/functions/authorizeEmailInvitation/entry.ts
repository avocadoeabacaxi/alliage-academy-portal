...
export default async function(req) {
  try {
    ...
    const { email, role, region, full_name, preferred_language } = await req.json();
    ...
    return Response.json({ success: true, email: normalizedEmail, preferred_language: selectedLanguage });
  } catch (error) {
